package com.mycom.game.users.service;
...
import com.mycom.game.users.dao.UsersDao;
...
public class usersServiceImpl implements UsersService{
	...
	public void login(ModelAndView mView, UsersDto dto, HttpSession session) {
		...
		//인자로 전달된 Dto에 있는 회원의 아이디를 이용해서 Select한다.
		UsersDto resultDto=dao.getData(dto.getId());
		//해당 아이디가 DB에 존재한다면
		if(resultDto!=null) {
			//사용자가 입력한 비밀번호와 DB에 저장된 암호화된 비밀번호를 비교해서 일치하는지 판단해야한다.
	...
	public void delete(ModelAndView mView, HttpSession session) {
		...
		mView.addObject("msg", id+"님 회원 탈퇴되었습니다.");
	}
